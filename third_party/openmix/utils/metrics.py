...
def calc_metrics(args, loader, label, label_onehot, model, criterion, use_second=False, use_max=True):
    ...
    if use_second:
        scores = 1 - np.partition(softmax, -2, 1)[:, -2]
        if use_max:
            scores += np.minimum(np.max(softmax, 1), 0.8)
    else:
        scores = np.max(softmax, 1) 
    # aurc, eaurc
    aurc, eaurc = calc_aurc_eaurc(softmax, correct, scores)
    # fpr, aupr
    auroc, aupr_success, aupr, fpr, tnr = calc_fpr_aupr(softmax, correct, scores)
...
# AURC, EAURC
def calc_aurc_eaurc(softmax, correct, scores):
    softmax = np.array(softmax)
    correctness = np.array(correct)

    sort_values = sorted(zip(scores[:], correctness[:]), key=lambda x: x[0], reverse=True)
...
# AUPR ERROR
def calc_fpr_aupr(softmax, correct, scores):
    softmax = np.array(softmax)
    correctness = np.array(correct)

    fpr, tpr, thresholds = metrics.roc_curve(correctness, scores)
    auroc = metrics.auc(fpr, tpr)
    idx_tpr_95 = np.argmin(np.abs(tpr - 0.95))
    fpr_in_tpr_95 = fpr[idx_tpr_95]
    tnr_in_tpr_95 = 1 - fpr[np.argmax(tpr >= .95)]

    precision, recall, thresholds = metrics.precision_recall_curve(correctness, scores)
    aupr_success = metrics.auc(recall, precision)
    aupr_err = metrics.average_precision_score(-1 * correctness + 1, -1 * scores)
