...
class PGD(Attack):
    ...
    def __init__(self, model, eps=8 / 255, alpha=2 / 255, steps=10, random_start=True, temp=1):
        super().__init__("PGD", model)
        self.eps = eps[:, None, None, None]
    ...
    def forward(self, images, labels, target_labels=None):
        ...
        for _ in range(self.steps):
            ...
            outputs = self.get_logits(adv_images) / self.temp
            ...
            delta = torch.min(self.eps, torch.max(-self.eps, adv_images - images))
...
class FGSM(Attack):
    ...
    def __init__(self, model, eps=8 / 255, temp=1):
    ...
    def forward(self, images, labels, target_labels=None):
        ...
        outputs = self.get_logits(images) / self.temp
        ...
        adv_images = images + self.eps[:, None, None, None] * grad.sign()
        adv_images = torch.clamp(adv_images, min=0, max=1).detach()
...
class PGDL2(Attack):
    ...
    def forward(self, images, labels, target_labels=None):
        ...
        if self.random_start:
            ...
            delta *= r / n * self.eps[:, None, None, None]
        ...
        for _ in range(self.steps):
            ...
            adv_images = adv_images.detach() + self.alpha[:, None, None, None] * grad
...
class DeepFool(Attack):
    ...
    def __init__(self, model, steps=50, overshoot=0.02, temp=1):
    ...
    def _forward_indiv(self, image, label):
        ...
        fs = self.get_logits(image)[0] / self.temp
...
class FAB(Attack):
    ...
    def get_diff_logits_grads_batch(self, imgs, la):
        ...
        with torch.enable_grad():
            y = self.get_logits(im) / self.temp
    ...
    def attack_single_run_targeted(self, x, y=None, use_rand_start=False):
        ...
        output = self.get_logits(x) / self.temp
